...
from models.dtuvideo import Dataset as Videoset
...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False):
        ...
        # self.scene_out_root = '/data0/zhl/neus_pro/outs/0311tvcg_neus/dtu/video'
        ...
        # self.scene_out_dir = os.path.join('video', case)
    ...
    def validate_image(self, idx=-1, resolution_level=-1, st=None, ed=None):
        ...
        # Process Geometry
        if st is None: frame_range = range(self.val_data.n_images)
        else: frame_range = range(st, ed)
        for idx in frame_range:
            ...
            intrinsic_matrix = self.val_data.intrinsics_all[idx].detach().cpu().numpy()
            transforms_matrix = self.val_data.pose_all[idx].detach().cpu().numpy()

            meta = {'intrinsic': intrinsic_matrix.tolist(),
                    'cam_transform_mat': transforms_matrix.tolist()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ed', type=int, default=None)
    ...
    runner.validate_image(st=args.st, ed=args.ed)
