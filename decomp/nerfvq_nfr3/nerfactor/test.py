...
def compute_rgb_scales():
    ...
    batch_path = join(config_ini[:-4], 'raw_test', 'ckpt-5')
...
def main(_):
    ...
    #--- Output Raw Results
    logger.info("Output Raw Results...")
    opt_scale = None
    ...
    outroot = outroot.replace('vis_test', 'raw_test')
    ...
    #--- Output Decomp Results
    logger.info("Output Decomp Results...")
    if data_type == 'nerf': opt_scale = compute_rgb_scales()
    else: opt_scale = None
    ...
    #--- Output Relit Results
    logger.info("Output Relit Results...")
    if data_type == 'nerf': opt_scale = compute_rgb_scales()
    else: opt_scale = None
    ...
    for batch_i, batch in enumerate(
        ...
        _, _, _, to_vis = vq_model.fast_render(
            batch, mode='test', relight_olat=True,
            relight_probes=True, opt_scale=opt_scale,
            ref_batch=True)
        # Visualize
        vq_model.vis_batch(to_vis, outdir_scaled, mode='test')
    ...
    logger.info("Output Segmentation Results...")
    if data_type == 'nerf': opt_scale = compute_rgb_scales()
    else: opt_scale = None
