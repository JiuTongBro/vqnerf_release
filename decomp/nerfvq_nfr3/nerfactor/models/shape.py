...
class Model(BaseModel):
    ...
    def _init_net(self):
        ...
        net['normal_mlp'] = mlp.Network(
            ...
            skip_at=[mlp_skip_at])
        net['normal_out'] = mlp.Network([mlp_width, mlp_width, 3,], act=['relu', 'relu', None,]) # normalized elsewhere
        return net
    ...
    def call(self, batch, mode='train'):
        ...
        id_, hw, _, rayd, _, alpha, _, xyz, normal, lvis = batch

        mask = alpha[:, 0] > 0
        rayd = tf.boolean_mask(rayd, mask)
        ...
        normal_pred = self._pred_normal_at(xyz, rayd)
        ...
        gt = {'normal': normal, 'alpha': alpha}
        loss_kwargs = {'normal_jitter': normal_jitter}
        # ------ To visualize
        to_vis = {'id': id_, 'hw': hw}
    ...
    def _pred_normal_at(self, pts, views, eps=1e-6):
        mlp_layers = self.net['normal_mlp']
        out_layer = self.net['normal_out']
        x_embed = self.embedder['xyz']
        v_embed = self.embedder['vdir']

        def chunk_func(inputs):
            surf, views = inputs[:, :3], inputs[:, 3:]
            surf_embed = x_embed(surf)
            feats = mlp_layers(surf_embed)
            view_embed = v_embed(views)
            feats = tf.concat([feats, view_embed], axis=-1)
            normals = out_layer(feats)
            return normals

        inputs = tf.concat([pts, views], axis=-1)
        normal = self.chunk_apply(chunk_func, inputs, 3, self.mlp_chunk)
