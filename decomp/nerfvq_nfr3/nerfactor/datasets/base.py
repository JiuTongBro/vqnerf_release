...
class Dataset():
    def __init__(
            self, config, mode, debug=False, shuffle_buffer_size=8,
            prefetch_buffer_size=None, n_map_parallel_calls=None):
        assert mode in ('train', 'vali', 'test', 'render'), (
            "Accepted dataset modes: 'train', 'vali', 'test', 'render', but input is %s"
        ) % mode
        self.config = config
        self.mode = mode
        self.debug = debug
    ...
    def build_pipeline(
            ...
            no_shuffle=False, pretrain=False, sort=True):
        cache = self.config.getboolean('DEFAULT', 'cache')
        is_train = self.mode == 'train'
        # Make dataset from files
        if sort: files = sorted(self.files)
        ...
        if pretrain:
            dataset = dataset.map(
                lambda x: self._process_example_precache(x, pretrain=pretrain),
                num_parallel_calls=self.n_map_parallel_calls)
        else:
            dataset = dataset.map(
                self._process_example_precache, # otherwise there's no param pretrain
                num_parallel_calls=self.n_map_parallel_calls)
