...
class Microfacet:
    ...
    """
    def __init__(self, default_rough=0.3, lambert_only=False):
        self.default_rough = default_rough
        self.lambert_only = lambert_only

    def __call__(self, pts2l, pts2c, normal, albedo=None, rough=None, f0=None):
        ...
        if f0 is None:
            f0 = 0.91 * tf.ones((tf.shape(pts2c)[0], 3), dtype=tf.float32)
        if rough is None:
        ...
        alpha = rough ** 2
        d = self._get_d(h, normal, alpha=alpha) # NxLx1
        g = self._get_g(pts2c, h, normal, alpha=alpha) # NxLx1
    ...
    def _get_g(v, m, n, alpha=0.1):
        ...
        return g[:, :, None] # (n_pts, n_lights, 1)

    @staticmethod
    def _get_d(m, n, alpha=0.1):
        """Microfacet distribution (GGX).
        """
        cos_theta_m = tf.einsum('ijk,ik->ij', m, n) # NxL
        ...
        return d[:, :, None] # (n_pts, n_lights, 1)

    def _get_f(self, l, m, f0 = None):
        """Fresnel (Schlick's approximation).
        """
        cos_theta = tf.einsum('ijk,ijk->ij', l, m)[:, :, None]
