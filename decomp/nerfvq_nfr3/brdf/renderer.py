...
def gen_light_xyz(envmap_h, envmap_w, envmap_radius=1e2):
    ...
    lngs, lats = np.meshgrid(lngs, lats) # (H,W)

    # To Cartesian
    rlatlngs = np.dstack((envmap_radius * np.ones_like(lats), lats, lngs))
    rlatlngs = rlatlngs.reshape(-1, 3) # (H*W)*(radius, lats, lngs)
    xyz = xm.geometry.sph.sph2cart(rlatlngs) # only related to the direction
    xyz = xyz.reshape(envmap_h, envmap_w, 3) # (h,w,3)

    # Calculate the area of each pixel on the unit sphere (useful for
    # integration over the sphere)
    sin_colat = np.sin(np.pi / 2 - lats) # sin(pi/2-x)=cos(x), (H,W)
    areas = 4 * np.pi * sin_colat / np.sum(sin_colat) # (H,W)
