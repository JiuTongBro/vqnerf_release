...
def _render(scene, outnode, result_socket, outpath, exr=True, alpha=True):
	...
	outnode.base_path = '/home/ma-user/work/zhl/tmp/%s/' % time()
...
def _render_ind(scene, outnode, result_socket, outpath, exr=True, alpha=True):
	bpy = preset_import('bpy', assert_success=True)
...
def _disable_cycles_mat_nodes_for_bi():
	...
	if bpy.context.scene.render.engine == 'BLENDER_RENDERER':
...
def render_rough(outpath, cam=None, obj_names=None, n_samples=1000, select=None):
	...
	logger.info("keys", scene.objects.node_tree.nodes.keys())
...
def render_lighting_passes(
	...
	all_passes = {  # a set useful for intrinsic image decomposition
		...
		'transmission_direct': 'TransDir', 'transmission_indirect': 'TransInd',
		'transmission_color': 'TransCol', 'emit': 'Emit', 'environment': 'Env'}
